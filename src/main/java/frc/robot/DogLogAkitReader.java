 ...
 */
public class DogLogAkitReader implements LogReplaySource {
    ...
    private Long recentRecordTimestamp;
    ...
    public DogLogAkitReader(String filename) {
    ...
    public boolean updateTable(LogTable table) {
        ...
        if (recentRecordTimestamp != null && (timestamp == null || recentRecordTimestamp > timestamp)) {
            table.setTimestamp(recentRecordTimestamp);
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (record.isControl()) {
                ...
                if (entry != null) {
                    System.out.println("entryVetted = " + entry);
                    System.out.println("timestamp = " + timestamp);
                    System.out.println("recent record timestamp = " + recentRecordTimestamp);
                    System.out.println("current entry timestamp = " + record.getTimestamp());

                    boolean isTimestampEntry = entry.equals("/Robot/SystemStats/EpochTimeMicros");

                    boolean firstrt = recentRecordTimestamp == null;
                    recentRecordTimestamp = record.getTimestamp();

                    if (firstrt && (timestamp == null || recentRecordTimestamp > timestamp)) {
                        table.setTimestamp(recentRecordTimestamp);
                    }
                    ...
                    } else if (timestamp != null && (record.getTimestamp() == timestamp || record.getTimestamp() == recentRecordTimestamp)) {
                        entry = entry.substring(1); // Remove leading slash
                        if (entry.startsWith("ReplayOutputs")) {
                            // Don't retrieve old replay outputs
                            continue;
                        }
                        System.out.println("admitted entry = " + entry);
